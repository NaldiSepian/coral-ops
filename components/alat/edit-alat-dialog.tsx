...
interface EditAlatDialogProps {
  ...
  onAlatUpdated: (alat: Alat) => void;
}

export function EditAlatDialog({ alat, open, onOpenChange, onAlatUpdated }: EditAlatDialogProps) {
  ...
  useEffect(() => {
    if (alat) {
      setFormData({
        ...
        stok_total: alat.stok_total.toString(),
        foto_url: alat.foto_url || ""
      });
      setFotoPreview(alat.foto_url || "");
    }
  }, [alat]);
  ...
  const validateForm = () => {
    ...
    // Prevent total from being lower than items currently borrowed
    const borrowed = alat ? alat.stok_total - alat.stok_tersedia : 0;
    if (alat && stokTotal < borrowed) {
      newErrors.stok_total = `Stok total tidak boleh kurang dari jumlah yang sedang dipinjam (${borrowed})`;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!alat || !validateForm()) return;

    setIsLoading(true);

    try {
      const response = await fetch(`/api/alat/${alat.id}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || "Failed to update alat");
      }

      const updatedAlat = await response.json();

      console.log("Berhasil: Alat berhasil diperbarui");

      onAlatUpdated(updatedAlat);
      onOpenChange(false);

    } catch (error) {
      console.error("Error updating alat:", error);
      alert(error instanceof Error ? error.message : "Gagal memperbarui alat");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Perbarui informasi alat. Stok total tidak boleh kurang dari stok tersedia saat ini.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-start gap-4">
              <Label htmlFor="edit-foto" className="text-right mt-2">
                Foto Alat
              </Label>
            ...
            {/* Deskripsi */}
            <div className="grid grid-cols-4 items-start gap-4">
              <Label htmlFor="edit-deskripsi" className="text-right mt-2">
                Deskripsi
              </Label>
              <Textarea
                id="edit-deskripsi"
            ...
            {/* Stok Total */}
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="edit-stok_total" className="text-right">
                Stok Total
              </Label>
              <div className="col-span-3">
                <Input
                ...
                {alat && (
                  <p className="text-xs text-muted-foreground mt-1">
                    Stok tersedia saat ini: {alat.stok_tersedia}
                  </p>
                )}
              </div>
            </div>
