...
export function ProgressDialog({
  ...
  const [includeToolPhotos, setIncludeToolPhotos] = useState(false);
  ...
  useEffect(() => {
    if (open) {
      ...
      setFileError(null);
      setLocationStatus(null);
      const defaultStatus = approvedReports === 0 ? "Menunggu" : "Hampir Selesai";
      setStatus(defaultStatus);
      setIncludeToolPhotos(defaultStatus === "Menunggu");
      setReturnTools(false);
      setPhotoPairs((pairs) => (pairs.length === 0 ? [createEmptyPair()] : pairs));
      
      // Initialize tool photos based on status
      if (defaultStatus === "Menunggu" && tools.length > 0) {
  ...
  useEffect(() => {
    if (status === "Menunggu") {
      setIncludeToolPhotos(true);
    } else if (status === "Selesai") {
      setIncludeToolPhotos(false);
    }
  }, [status]);

  useEffect(() => {
    if (includeToolPhotos && tools.length > 0) {
      const activeTools = tools.filter(tool => !tool.is_returned);
      setToolPhotos(activeTools.map(tool => ({
        alat_id: tool.alat_id,
        file: null,
  ...
  const handleSubmit = async () => {
    ...
    // Validasi foto pengambilan alat
    if (includeToolPhotos && tools.length > 0) {
  ...
  const reportLabel = `Laporan Progres Ke-${existingReports + 1}`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5">
          ...
          {tools.length > 0 && status !== "Selesai" && (
            <div className="space-y-3">
              {status !== "Menunggu" && (
                <div className="flex items-start gap-3 rounded-lg border p-3">
                  <Checkbox
                    id="include-tool-photos"
                    checked={includeToolPhotos}
                    onCheckedChange={(checked) => setIncludeToolPhotos(checked === true)}
                  />
                  <label htmlFor="include-tool-photos" className="text-sm cursor-pointer">
                    Sertakan foto pengambilan alat
                  </label>
                </div>
              )}
              {includeToolPhotos && (
