...
export function AssignmentCard({
  ...
  const approvedReports = assignment.laporan_progres?.filter(report => report.pelapor_id === userId && report.status_validasi === "Disetujui").length || 0;
  
  // Find the user's own reports
  const userReports = assignment.laporan_progres?.filter(report => report.pelapor_id === userId) || [];
  const userLatestReport = userReports[0];
  const userFinalReport = userReports.find(report => report.status_progres === "Selesai");
  const isUserFinalApproved = userFinalReport?.status_validasi === "Disetujui";
  ...
  const isLatestRejected = userLatestReport?.status_validasi === "Ditolak";
    ...
    <Card className="p-4 flex flex-col gap-4 border shadow-sm">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        ...
        <Button
          ...
          disabled={!canReport}
        >
          {canReport ? progressLabel : isLatestRejected ? "Laporan Ditolak" : isUserFinalApproved ? "Menunggu Penyelesaian" : "Menunggu Validasi"}
        </Button>
        <Button
          onClick={onKendala}
          variant="outline"
