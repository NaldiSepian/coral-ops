...
export default function PenugasanPage() {
  ...
  useEffect(() => {
    const fetchPenugasan = async () => {
      try {
        ...
        const response = await fetch(`/api/penugasan?${params}`);
  ...
  const handleViewDetail = (id: number) => {
    router.push(`/views/spv/penugasan/${id}`);
  };

  const handleCreateNew = () => {
    router.push('/views/spv/penugasan/buat');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col space-y-4 xl:flex-row xl:items-center xl:justify-between xl:space-y-0">
        <div>
          ...
          <p className="text-muted-foreground">Kelola semua penugasan teknisi</p>
        </div>
        <button
          onClick={handleCreateNew}
          className="rounded-md bg-primary px-4 py-2 text-primary-foreground hover:bg-primary/90 w-full sm:w-auto xl:px-6 xl:py-3"
        >
          Buat Penugasan Baru
        </button>
      </div>
