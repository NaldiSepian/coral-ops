...
export default function PenugasanDetailPage() {
  ...
  const handleComplete = async () => {
    ...
    if (confirm('Apakah Anda yakin ingin menandai penugasan ini sebagai selesai?')) {
      try {
        const response = await fetch(`/api/penugasan/${penugasanId}/approve-selesai`, {
          method: 'POST',
        });

        if (!response.ok) {
          const data = await response.json();
          throw new Error(data.error || 'Failed to complete penugasan');
        }
        ...
        setError({
          message: err instanceof Error ? err.message : 'Gagal menyelesaikan penugasan',
          code: 'UPDATE_ERROR'
        });
      }
    }
