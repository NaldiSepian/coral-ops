...
export async function GET(
  ...
  try {
      ...
      .select(`
        *,
        ...
        laporan_progres(
          ...
          catatan_validasi,
          pelapor_id,
          pelapor:profil!pelapor_id(nama),
          bukti_laporan(
            id,
            judul,
            deskripsi,
    ...
    // Ekstrak foto alat dan pengembalian dari JSONB array di alat
    const toolPhotosData = (penugasan.alat || []).flatMap((a: any) => {
      if (!a.foto_ambil || !Array.isArray(a.foto_ambil)) return [];
      return a.foto_ambil.map((f: any) => ({
        alat_id: a.alat_id,
        foto_url: f.url,
    ...
    const returnToolPhotosData = (penugasan.alat || []).flatMap((a: any) => {
      if (!a.foto_kembali || !Array.isArray(a.foto_kembali)) return [];
      return a.foto_kembali.map((f: any) => ({
        alat_id: a.alat_id,
        foto_url: f.url,
        created_at: f.created_at,
        uploaded_by: f.uploaded_by,
    ...
    // Filter reports: Hanya milik user login (Strict Private)
    const userReports = (penugasan.laporan_progres || [])
      .filter((report: any) => report.pelapor_id === user.id);

    // Tampilkan semua alat untuk tim yang ditugaskan
    const userTools = penugasan.alat || [];

    // Add tool photos to response
    const processedPenugasan = {
      ...penugasan,
      alat: userTools,
    ...
    if (processedPenugasan.laporan_progres && Array.isArray(processedPenugasan.laporan_progres)) {
      processedPenugasan.laporan_progres = processedPenugasan.laporan_progres.sort(
    ...
    if (processedPenugasan.laporan_progres && Array.isArray(processedPenugasan.laporan_progres)) {
      processedPenugasan.laporan_progres = processedPenugasan.laporan_progres.map((report: any) => {
        if (report.titik_gps) {
    ...
    return NextResponse.json({ data: processedPenugasan });
  } catch (error) {
