...
export async function POST(
  ...
  try {
    ...
    // Handle foto pengambilan alat — append ke JSONB array
    if (tool_photos) {
      ...
      for (const toolPhoto of tool_photos) {
        ...
        const currentPhotos: any[] = (currentRow?.foto_ambil as any[]) || [];
        const newEntry = {
          url: toolPhoto.foto_url,
          uploaded_by: user.id,
        ...
        const { error: updateToolError } = await supabase
          .from('peminjaman_alat')
          .update({ 
            foto_ambil: [...currentPhotos, newEntry],
            peminjam_id: user.id
          })
          .eq('penugasan_id', penugasanId)
          .eq('alat_id', toolPhoto.alat_id)
        ...
        if (updateToolError) {
          console.error('Failed to append tool pickup photo:', updateToolError);
          return NextResponse.json({ error: "Gagal menyimpan foto pengambilan alat" }, { status: 500 });
        }
      }
    }

    // Handle foto pengembalian alat — append ke JSONB array
    if (shouldAutoReturn && return_tool_photos) {
      ...
      for (const returnToolPhoto of return_tool_photos) {
        ...
        const currentPhotos: any[] = (currentRow?.foto_kembali as any[]) || [];
        const newEntry = {
          url: returnToolPhoto.foto_url,
          uploaded_by: user.id,
          created_at: new Date().toISOString(),
        };
        ...
        const { error: updateReturnToolError } = await supabase
          .from('peminjaman_alat')
          .update({ foto_kembali: [...currentPhotos, newEntry] })
          .eq('penugasan_id', penugasanId)
          .eq('alat_id', returnToolPhoto.alat_id)
          .eq('is_returned', false);

        if (updateReturnToolError) {
          console.error('Failed to append tool return photo:', updateReturnToolError);
    ...
    if (shouldAutoReturn) {
      ...
      if (!activeLoansError && activeLoans && activeLoans.length > 0) {
        for (const loan of activeLoans) {
          ...
          // Ambil array foto kembali saat ini
          const { data: currentRow } = await supabase
            .from('peminjaman_alat')
            .select('foto_kembali')
            .eq('id', loan.id)
          ...
          const { error: updateLoanError } = await supabase
            ...
            .update({
              is_returned: true,
              returned_at: nowIso,
              foto_kembali: [...currentPhotos, newEntry],
            })
            .eq('id', loan.id);
